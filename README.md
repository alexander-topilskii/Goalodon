...
Формат дня — YAML frontmatter (`date`) и несколько целей:
...
У каждой цели свой план и свой чеклист. Строка `Проект: …` относит цель к длинной серии (подтягивания, язык). Без проекта цель — точечная задача на один день. Строки `>` под задачей — напоминание. Старые файлы с `goal` в YAML читаются как одна цель. Frontmatter через `js-yaml` (FAILSAFE), без `gray-matter`.
...
Сайт — PWA: в Chrome на телефоне можно поставить как приложение (иконка на рабочем столе, без строки браузера). Нужен задеплоенный HTTPS (GitHub Pages), не `npm run dev`.

1. Откройте `https://<owner>.github.io/Goalodon/` в Chrome.
2. Нажмите **Установить** на баннере или в Настройках. Если кнопки нет: меню Chrome **⋮ → Установить приложение**.
3. Токен GitHub остаётся в этом Chrome (тот же `localStorage`).
